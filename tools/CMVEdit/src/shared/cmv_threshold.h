...
template <class cmap_t,class image,int bits_y,int bits_u,int bits_v>
    void ThresholdImageYUV(cmap_t *cmap,const image &img,const cmap_t *tmap)
{
...
//  const yuv_generic<unsigned char> *buf = img.buf;
  const yuv* buf = img.buf;

  for(int i=0; i<size; i++){
    uchar py = buf[i].y >> rshift_y;
    uchar pu = buf[i].u >> rshift_u;
    uchar pv = buf[i].v >> rshift_v;
