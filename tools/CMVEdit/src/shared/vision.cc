...
//   CMVision::SaveThresholdFile(tmap,num_y,num_u,num_v,tmapfile);
  ...
  reg  = new CMVision::region[max_regions];
...
#ifdef YUVPIXEL
  CMVision::ThresholdImageYUV
    <cmap_t,vision_image,bits_y,bits_u,bits_v>
    (cmap,img,tmap);
#else
  if(false){
...
//
//  //printf("found %i runs, %i regions\n",num_runs,num_regions);
...
bool LowVision::getThresholdImage(SImage &img)
{
  if(img.allocate(width,height)){
    IndexToRgb(img.img,cmap,width,height,color,num_colors);
    return(true);
  }else{
    return(false);
  }
...
CMVision::region *LowVision::findRegion(int x,int y)
{
...
cmap_t LowVision::getTMap(yuv c)
{
  int yi = c.y >> (8-bits_y);
  int ui = c.u >> (8-bits_u);
  int vi = c.v >> (8-bits_v);
  return(getTMapFast(yi,ui,vi));
}
...
bool LowVision::loadTMap(const char *tmapfile) const
{
  return(CMVision::LoadThresholdFile(
...
bool LowVision::makeRgbImage(vision_image &src,SImage &dest)
{
  //YuvToRgbInit();
  if(!dest.allocate(src.width,src.height)) return(false);
#ifdef YUVPIXEL
  dest.img = colorSpaceConverter::YUVtoRGB(src.buf,src.width,src.height);
#else
  dest.img = colorSpaceConverter::YUYVtoRGB(src.buf,src.width,src.height);
#endif
  return(true);
}
...
void RegionFilter::init(const CMVision::region *reg_list)
{
  reg = reg_list;
...
const CMVision::region *RegionFilter::getNext()
{
...
bool RegionFilter::operator()(const CMVision::region &reg) const
{
