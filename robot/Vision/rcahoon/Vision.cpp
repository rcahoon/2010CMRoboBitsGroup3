		...
		if (rle[k].rank >= 0)
		{
			...
			case VisionObject::YellowGoalPost:
					...
					iter != y_goals.end(); iter++)
				{
					...
					if ( max(r1.x2, r2.x2)-min(r1.x1, r2.x1) < r1.x2 - r1.x1 + r2.x2 - r2.x1 + 15 &&
	...
	const HMatrix* transform, VisionFeatures & outputVisionFeatures)
{
	...
	obj->setConfidence(100.0f/(100+position.length()));
...
void Vision::findObjects(const HMatrix* transform, VisionFeatures & outputVisionFeatures)
{
	...
	{
		pixel_run& r = **iter;
		if (r.area < classes[VisionObject::Ball].min_size) continue;
	...
	{
		pixel_run& r = **iter;
		if (r.area < classes[VisionObject::BlueGoalPost].min_size) continue;

		/*unsigned goalHist[320];
		mzero(goalHist, 320);
		for(int k=row_starts[r.y1]; k < row_starts[r.y2+1]; k++)
		{
			if (rle[k].canon() != &r)
	...
	{
		pixel_run& r = **iter;
		if (r.area < classes[VisionObject::YellowGoalPost].min_size) continue;
	...
	{
		pixel_run& r = **iter;
		//if (r.area < classes[VisionObject::Line].min_size) continue;
