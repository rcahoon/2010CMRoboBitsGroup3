...
#ifndef ROBOCUP2010___CMV_THRESHOLD_H__
#define ROBOCUP2010___CMV_THRESHOLD_H__

#include "cmv_types.h"

#include "shared/iofile.h"
...
template <class cmap_t,class pixel>
inline void ThresholdImage(cmap_t *cmap, pixel* buf ,cmap_t *tmap) 
{
    pixel p;
    int m;
...
//    {
//        p = buf[i / 2];
//        m = ( p.u << 8/*lshift_u*/) + p.v;
//        cmap[ i ] = tmap[m + ((p.y1 >> 4/*rshift_y*/) << 16/*lshift_y*/)];
//        cmap[++i] = tmap[m + ((p.y2 >> 4/*rshift_y*/) << 16/*lshift_y*/)];
...
#ifdef LOGGING
	LOG_DEBUG("img:%dx%d xy(%d,%d) cmap=%d",
...
int CheckTMapColors(tmap_t *tmap,int num_y,int num_u,int num_v,int colors,int default_id)
{
  ...
  if(n > 0) {
#ifdef LOGGING
//    LOG_WARN("%d TMap errors found.", n)
#endif
  }
...
template <class pixel_t,class cmap_t,class image,int bits_y,int bits_u,int bits_v>
void MarkTMapUsedYUV(cmap_t *tmap,const image &img,cmap_t mark,
                  int x,int y,int w,int h)
{
  pixel_t *buf,p;
  ...
  for(int yi=y; yi<y+h; yi++){
    for(int xi=x; xi<x+w; xi+=2){
      p = buf[yi*img.width + xi];
    uchar py = p.y >> rshift_y;
    uchar pu = p.u >> rshift_u;
    uchar pv = p.v >> rshift_v;
